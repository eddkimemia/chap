...
import { Phone, CheckCircle2, RefreshCw, ArrowRight, Shield, Smartphone } from 'lucide-react'
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch('/api/auth/verify-otp', {
        ...
        body: JSON.stringify({ code, type: 'phone', userId }),
      })
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <Card className="w-full max-w-md glass-card shadow-premium-xl rounded-3xl border-0">
          <CardContent className="p-8 sm:p-10 text-center">
            ...
            <h1 className="text-2xl font-bold text-navy">Phone Verified!</h1>
            <p className="text-muted-foreground mt-1 text-sm">Your phone number has been verified.</p>
      ...
      <Card className="w-full max-w-md glass-card shadow-premium-xl rounded-3xl border-0">
        <CardContent className="p-8 sm:p-10">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-navy tracking-tight">Verify Your Phone</h1>
          ...
          <form onSubmit={handleVerify} className="space-y-5">
            ...
            <Button type="submit" disabled={loading || code.length !== 6} className="w-full h-12 rounded-2xl bg-royal text-white border-0 shadow-lg shadow-royal/20">
              {loading ? <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><CheckCircle2 className="h-4 w-4 mr-2" /> Verify Phone</>}
            </Button>
          ...
          <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-amber-800">Standard SMS rates may apply. Your phone number is used for account security and transaction notifications only.</p>
            </div>
          </div>
