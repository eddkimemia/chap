...
export function SellerProfileClient({ sellerId }: { sellerId: string }) {
  ...
  const fetchListings = () => {
    ...
    const params = new URLSearchParams({ status: 'active', limit: '24' })
    if (featuredFilter) params.set('featured', 'true')
    fetch(`/api/users/${sellerId}/listings?${params}`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/users/${sellerId}/listings?status=active&limit=24`).then((r) => r.json()),
          ...
          >
            <div className="glass-card rounded-3xl p-6 sticky top-24">
              ...
              {(seller.profile?.website || (seller.profile?.socialLinks && seller.profile.socialLinks !== '{}')) && (
                <div className="mb-6">
                  <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Links</h3>
                  <div className="flex flex-wrap gap-2">
                    {seller.profile?.website && (
                      <a href={seller.profile.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-royal transition-colors bg-slate-100 hover:bg-royal/10 px-3 py-1.5 rounded-lg">
                    ...
                    {(() => {
                      try {
                        const links = JSON.parse(seller.profile?.socialLinks || '{}')
                        ...
                        return Object.entries(links).map(([platform, url]) =>
                          url ? (
                            <a key={platform} href={url as string} target="_blank" rel="noopener noreferrer"
                              ...
                              style={{ background: socialBg[platform] || '#64748b' }}>
                              {socialIcon[platform] || <Globe className="h-3.5 w-3.5" />}
                            </a>
                          ) : null
                        )
            ...
            {activeTab === 'listings' && (
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                      !featuredFilter
                        ? 'bg-royal text-white'
                        : 'bg-white text-slate-500 border border-slate-200 hover:border-royal/30'
                    }`}
                  >
                    All
                  </button>
                ...
                {listingsLoading ? (
                  ...
                  <div className="text-center py-12 rounded-2xl bg-white/60 backdrop-blur-sm border border-slate-100">
                    <Package className="h-10 w-10 mx-auto text-slate-300 mb-2" />
                    <p className="text-slate-500 font-medium text-sm">
                      {featuredFilter ? 'No featured listings' : 'No active listings yet'}
                    </p>
                  </div>
                )}
              </div>
